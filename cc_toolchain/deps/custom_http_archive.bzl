...
def _download_remote_files(ctx, auth = None):
    ...
    for path, remote_file_urls in ctx.attr.remote_file_urls.items():
        ...
        o = path[:path.rindex('/')] if "/" in path else path              # Does it work for Windows?
        ctx.extract(path, output = o)
...
def _custom_http_archive_impl(ctx):
...
custom_http_archive = repository_rule(
    implementation = _custom_http_archive_impl,
